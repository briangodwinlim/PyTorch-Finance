...
class StockDataset(Dataset):
    def __init__(self, start_date, end_date, sequence_length, transform_spec=dict(), force_reload=False,
                 raw_path='dataset/raw', cache_path='dataset/cached', data_file='sample_data.csv'):
        ...
        if force_reload or not os.path.exists(os.path.join(self.cache_path, cache_file)):
            self.features, self.targets, self.features_transform, self.targets_transform, self.index = self.process_data()
            os.makedirs(self.cache_path, exist_ok=True)
            with open(os.path.join(self.cache_path, cache_file), 'wb') as file:
                pickle.dump((self.features, self.targets, self.features_transform, self.targets_transform, self.index), file)
                
        # Load cached file if available and not force_reload
        else:
            with open(os.path.join(self.cache_path, cache_file), 'rb') as file:
                self.features, self.targets, self.features_transform, self.targets_transform, self.index = pickle.load(file) 
    ...
    def process_data(self):
        ...
        raw_data = pd.read_csv(os.path.join(self.raw_path, self.data_file), parse_dates=['dt'], index_col=['dt'])
        ...
        index = processed_data.index[self.sequence_length:]                             # Get index of targets
        ...
        return features, targets, features_transform, targets_transform, index
    ...
    def __len__(self):
        return self.targets.shape[0] - self.sequence_length
